...
for status in statuses:
    if status[2] not in achs:
        achs[status[2]] = 0
    lst = status[5].split(',')
    if status[2] in achs and lst[0] != '[]' and len(lst) > 1:
        achs[status[2]] += len(status[5].split(','))

sortedGroupAndCounts = dict(sorted(achs.items(), key=lambda x: x[1]))
